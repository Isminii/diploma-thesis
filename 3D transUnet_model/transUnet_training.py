...
def plot_metrics(train_losses, val_losses, val_dice_scores, val_iou_scores, num_epochs):
    ...
    plt.tight_layout()
    plt.savefig("training_metrics_embeddim512_heads4_splits1.png")
...
if __name__ == "__main__":
    ...
    model = TransUNet3D(in_channels=4, base_channels=32, img_shape=(128, 128,128), embed_dim=256, depth=4, heads=4, mlp_dim=512, num_classes=4,dropout=0.1)

    num_params, num_bytes = model_memory_info(model)
    print(f"Model parameters: {num_params}, Memory required: {num_bytes:.2f} MB")

    optimizer = optim.AdamW(params=model.parameters(), lr=0.0005,weight_decay=1e-4)
    ...
    checkpoint_dir = "transUnet_checkpoints_justckeck"
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}/{num_epochs} [Training]", leave=False):
            data, target = batch  # data: (B, 4, D, H, W), target: (B, 4, D, H, W)
            data = data.to(device)
            target = target.to(device)

            optimizer.zero_grad()
            outputs = model(data)  # outputs: (B, 4, D, H, W)
            outputs_resized = F.interpolate(outputs, size=target.shape[2:], mode='trilinear', align_corners=False)
